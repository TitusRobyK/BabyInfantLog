...
describe('invitation email content', () => {
  it('provides Parent B with the complete joining path without putting the code in the URL', () => {
    const email = buildInviteEmail('7K3QP', '2026-07-13T18:00:00.000Z', 'https://babylog.example.com', 'Titus')

    expect(email.subject).toBe('Titus invited you to join Baby Log')
    expect(email.text).toContain("Titus invited you to join your family's private space in Baby Log.")
    ...
    expect(email.text).toContain('ask Titus to generate a new one from Settings.')
  ...
  it('escapes the verified parent name before adding it to HTML', () => {
    const email = buildInviteEmail(
      ...
      'Titus <script>alert(1)</script>',
    )

    expect(email.text).toContain('Titus <script>alert(1)</script> invited you')
    expect(email.html).toContain('Titus &lt;script&gt;alert(1)&lt;/script&gt; invited you')
